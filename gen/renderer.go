...
func (r Renderer) Render(ctx context.Context, provider string) error {
	outputDirectory := filepath.Join(r.OutputDirectory, provider)

	{
		...
		remoteCRDs, err = r.patchCRDs(remoteCRDs)
		if err != nil {
			return microerror.Mask(err)
		}

		err = writeCRDsToDirectory(outputDirectory, remoteCRDs)
		if err != nil {
			return microerror.Mask(err)
		}
	}
...
func (r Renderer) downloadRepositoryCRDs(ctx context.Context, repo RemoteRepositoryDefinition) ([]v1.CustomResourceDefinition, error) {
	...
	for _, entry := range targetEntries {
		...
		for _, crd := range crds {
			if !repo.FilterCategory || contains(crd.Spec.Names.Categories, repo.Provider) {
				allCrds = append(allCrds, crds...)
			}
		}
	}
